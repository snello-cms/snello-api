...
@Controller(URL_MAP_RULES_PATH)
public class UrlMapRuleController {
    @Inject
    ApiService apiService;

    static String table = URL_MAP_RULES;
    ...
    Logger logger = LoggerFactory.getLogger(UrlMapRuleController.class);
    ...
    public HttpResponse<?> post(@Body String body) throws Exception {
        ...
        eventPublisher.publishEvent(new UrlMapRuleCreateUpdateEvent(map));
    ...
    public HttpResponse<?> delete(HttpRequest<?> request, @NotNull String uuid) throws Exception {
        ...
        if (result) {
            eventPublisher.publishEvent(new UrlMapRuleDeleteEvent(uuid));
            return ok();
        }
        return serverError();
    }
