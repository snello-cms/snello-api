...
@Controller(AppConstants.DRAGGABLES_PATH)
public class DraggablesController {
    ...
    public HttpResponse<?> post(@Body String body) throws Exception {
        ...
        eventPublisher.publishEvent(new DraggableCreateUpdateEvent(map));
        return ok(map);
    }
    ...
    public HttpResponse<?> delete(HttpRequest<?> request, @NotNull String uuid) throws Exception {
        ...
        if (result) {
            eventPublisher.publishEvent(new DraggableDeleteEvent(uuid));
            return ok();
        }
        return serverError();
    }
